...
junit_function () {
    ...
    echo -e "\n  *** `date +"%a %d/%b/%Y %H:%M:%S UTC%z"` Running JUnit tests ***\n  "
    ...
    #
    echo "$`date +"%a %d/%b/%Y %H:%M:%S UTC%z"` (start of job) MARK_BUILD_AS_UNSTABLE (this text will be cleared if no problems found)" > ${WORKSPACE}/post_build_status_marker.txt
    ...
    echo "`date +"%a %d/%b/%Y %H:%M:%S UTC%z"` (after build and tests) no need for Jenkins Text-finder plugin to override the build status" > ${WORKSPACE}/post_build_status_marker.txt
    ...
    RETVAL=$?
    if [ "${RETVAL}" != "0" ]; then
      echo -e "`date +"%a %d/%b/%Y %H:%M:%S UTC%z"` (after build and tests) MARK_BUILD_AS_UNSTABLE (set by check_if_JVM_abend.py)" > ${WORKSPACE}/post_build_status_marker.txt
    fi
