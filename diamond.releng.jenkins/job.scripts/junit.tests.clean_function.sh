...
junit_tests_clean_function () {
    ...
    if [[ -d "${materialize_workspace_path}_git" ]]; then
        ...
        find ${materialize_workspace_path}_git -type d \( -name "test-reports" -o -name "test-scratch" \) -print0 | sort --zero-terminated | xargs --null --no-run-if-empty -I % -L 1 --max-procs 20 -n 1 bash -c 'echo "deleting $1"; rm -rf "$@"' _
        RETVAL=$?
