...
if [ -n "${copyartifact_build_number}" ]; then
    echo "append-build-description: triggering ${platform_count} test jobs of <a href=\"/job/${upstream_create_product_job}/\">create-product${squish_platform_job_suffix}</a> build <a href=\"/job/${upstream_create_product_job}/${copyartifact_build_number}/\">${copyartifact_build_number}</a>"
fi
