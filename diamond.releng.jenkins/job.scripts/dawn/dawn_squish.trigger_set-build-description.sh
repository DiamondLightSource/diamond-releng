...
if [ -n "${copyartifact_build_number}" ]; then
    echo "set-build-description: triggering ${platform_count} test jobs of <a href=\"/job/${Dawn_upstream_product_job}/\">create-product${Dawn_squish_platform_job_suffix}</a> build <a href=\"/job/${Dawn_upstream_product_job}/${copyartifact_build_number}/\">${copyartifact_build_number}</a>"
fi
