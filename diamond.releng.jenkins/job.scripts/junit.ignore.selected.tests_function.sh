...
junit_ignore_selected_tests () {
    ...
    if [[ -n "${testnames_to_ignore_pattern}" ]]; then
        if [[ -d "${materialize_workspace_path}_git" ]]; then
            find ${materialize_workspace_path}_git -type f -regextype posix-extended -regex ".*/TEST-${testnames_to_ignore_pattern}\.xml" || true
            find ${materialize_workspace_path}_git -type f -regextype posix-extended -regex ".*/TEST-${testnames_to_ignore_pattern}\.xml" -exec mv -v {} {}.IGNORED \; || true
        fi
    fi
