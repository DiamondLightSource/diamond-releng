...
def parse_jenkins_jobname(jobname):
    ...
    if jm:
        ...
        if 'create.product' in jobname:
            ...
            # -Dskip_ALL_test_fragments=true  retained for versions prior to Dawn 2.2 / GDA 9.2
...
def test_parse_jenkins_jobname():
    ...
    p = parse_jenkins_jobname('GDA.master-gerritbeta-trigger')
    assert p == [
        ('download.public', False),
        ('GDA_release', 'master'),
        ('job_variant', None),
        ('gerrit_job_to_trigger', 'GDA.master-junit.tests-gerritbeta'),
        ]
    write_parse_result(p, output)
