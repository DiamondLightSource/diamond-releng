...
if [ -n "${copyartifact_build_number}" ]; then
    echo "append-build-description: testing <a href=\"/job/${GDA_upstream_product_job}/\">create-product</a> build <a href=\"/job/${GDA_upstream_product_job}/${copyartifact_build_number}/\">${copyartifact_build_number}</a>"
fi
