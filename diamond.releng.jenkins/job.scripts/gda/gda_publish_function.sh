        ...
        for platform in linux64 windows64 mac64; do
            ...
            if [[ "${!platform__indirect}" == "true" ]]; then
                ...
                if [[ "${publish_webserver_zip_remove_old_versions}" == "true" ]]; then
                    ...
                    ${rsync} -ilprtDOv --include "/*-${platform}.zip" --exclude '*' --delete ${WORKSPACE}/artifacts_to_publish/ ${publish_webserver_zip_directory_parent}/builds-${name_to_publish_as}
                else
                    ${rsync} -ilprtDOv --include "/*-${platform}.zip" --exclude '*' ${WORKSPACE}/artifacts_to_publish/ ${publish_webserver_zip_directory_parent}/builds-${name_to_publish_as}
                fi
