...
Testing notes:
   Set environment variables something like this:
      ...
      export gerrit_verified_option="post Gerrit VERIFIED=+1/-1 ... depending on whether test job passes/fails"
      export gerrit_verified_option="post Gerrit VERIFIED=+1 ... if test job passes, otherwise no change"
      export gerrit_verified_option="post Gerrit VERIFIED=-1 ... if test job fails, otherwise no change"
      export gerrit_verified_option="leave Gerrit verified status unchanged, but post a comment with build result"
...
class RequestedChangesProcessor():
    ...
    def get_changes_from_gerrit(self):
        ...
        for i in range(1, MAX_TOPICS+1):
            ...
            url = 'changes/?q=topic:{%s}+-status:abandoned&o=CURRENT_REVISION&o=DETAILED_ACCOUNTS&o=LABELS' % (urllib.quote(build_param_value),)
        ...
        for i in range(1, MAX_CHANGESETS+1):
            ...
            try:
                ...
                self.errors_found = True
                continue
            url = 'changes/?q=change:%s&o=CURRENT_REVISION&o=DETAILED_ACCOUNTS&o=LABELS' % (urllib.quote(build_param_value),)
...
''')
    ...
    def generate_gerrit_ssh_command(self):
        ...
        self.ssh_fetched_changes = []  # list of [ssh command, boolean if change is currently verified, text describing additional changes implicitly included] for the change we fetched for each repo
        self.ssh_ancestor_changes = []  # list of ssh commands for unmerged older commits that were implicitly tested
        for ci in self.changes_to_fetch:
            try:
                change_is_currently_verified = ci[0]['labels']['Verified']
            except:
                # Verified label might not apply here
                change_is_currently_verified = False
            self.ssh_fetched_changes.append(
                ['%s gerrit review -t autogenerated:Jenkins:DLS %s,%s -p %s -n NONE' % (self.gerrit_ssh_command, ci[0]['_number'], ci[0]['revisions'].values()[0]['_number'], ci[0]['project']),
                 change_is_currently_verified,
                 '']
                )
            if len(ci) > 1:  # if the fetched change includes older unmerged changes in its dependency chain
                ...
                for cr in ci[1:]:
                    self.ssh_ancestor_changes.append(
                        ['%s gerrit review -t autogenerated:Jenkins:DLS %s,%s -n NONE' % (self.gerrit_ssh_command, cr['_change_number'], cr['_revision_number']),  # project not in record
        ...
        with open(NOTIFY_GERRIT_AT_START_FUNCTION_FILE_PATH, 'w') as started_script_file:
            ...
            for c in self.ssh_fetched_changes:
                started_script_file.write('  %s --message \'"Build Started%s %s"\''
                                  % (c[0], c[2],  os.environ.get('BUILD_URL','')))
                if c[1] and "status unchanged" not in self.gerrit_verified_option:
